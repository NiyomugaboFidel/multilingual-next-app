...
const Icon:React.FC<IconProps> = ({iconName , className, iconTag, icontype = true , width= 16.5 , height= 16.5}) => {
  ...
  return (
    <span className={` cursor-pointer ${ className } ${icontype ? icon1 : ''}`}>
     {iconTag ? iconTag  : <Image width={width} height={height} src={`/icons/${iconName}`} alt={iconName ? iconName : 'icon'} className="w-auto h-auto bg-no-repeat" priority />}
    </span>
  )
}
