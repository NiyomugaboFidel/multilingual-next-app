    ...
    },
    {
      ...
      items: [
        ...
        items: [
          ...
          { name: 'Computer Accessories', link: '/category/computers-laptops/accessories' },
        ],
      },
      {
        ...
        items: [
          { name: 'Digital Cameras', link: '/category/cameras-photo-video/digital-cameras' },
          { name: 'Video Cameras', link: '/category/cameras-photo-video/video-cameras' },
          { name: 'Camera Lenses', link: '/category/cameras-photo-video/lenses' },
          { name: 'Tripods & Accessories', link: '/category/cameras-photo-video/tripods' },
        ],
      },
      {
        title: 'Headphones',
        ...
        items: [
          ...
          { name: 'Noise-Canceling Headphones', link: '/category/headphones/noise-canceling' },
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <nav className="mb-4">
          <ol className="flex items-center space-x-2 text-bodySmall text-gray-500">
            <li>
              <Link href="/" className="hover:text-gray-700 text-bodySmall text-textColor-dark">
                Home
              </Link>
            </li>
            <li>{"/"}</li>
            <li className="text-gray-700 text-bodySmall">Categories</li>
          </ol>
        </nav>
  
        {/* Title */}
        <h1 className="text-headingH3 text-textColor-dark font-[600]">Shop categories</h1>
  
        {/* Brands */}
        <div className=" py-[24px] grid items-center grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-[20px] transition-all ease-in-out duration-300">
         {
          CompanyData.map((company, index) => (
          <Image src={company.logo} alt={company.label} width={200} height={100} className='w-auto h-auto' />
          ))}
          ...
          {categories.map((category) => (
            ...
            >
              ...
              <Link href={`/category/${category.title.toLowerCase()}`}>
                <div className="relative h-48  bg-gray-50 rounded-[6px]">
                  <Image
                    src={category.image}
                    alt={category.title}
                    width={300}
                ...
                <h3 className='text-pretty text-bodyDefault font-[600] p-2'>{category.title}</h3> 
              </Link>
                ...
                <ul className="space-y-2">
                  {category.items.map((item) => (
                    <li key={item.name}>
                      <Link
                        href={item.link}
                        className="text-Gary-700 text-bodySmall hover:text-gray-900 transition-colors block  rounded-[6px] hover:bg-Gary-100  "
                      >
                        {item.name}
                      </Link>
                    </li>
